...
import { tableName } from "../../../static/constants/combinedOrders";
...
export default class BoardBodyOptionsComponent extends Component {
    ...
    render() {
        const { table, data, searchValue, getData, toggelRemoved, updateSearchBar } = this.props;
        const date = new Date();

        return (
            <BoardBodyOptions table={table}>
                <StandardButton ml={3} onClick={getData}>Refresh</StandardButton>
                <StandardButton ml={3} onClick={toggelRemoved}>Toggle Removed</StandardButton>
