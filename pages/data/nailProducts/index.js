...
import {
    NAIL_PRODUCTS_COLUMN_DESCRIPTION,
    NAIL_PRODUCTS_COLUMN_PROPERTIES,
    NAIL_PRODUCTS_COLUMN_PROPERTIES_TYPE,
    pathName,
    tableName,
    endpoint
} from '../../../static/constants/nailProducts';
...
class BoardJsx extends React.Component {
    ...
    getData = async () => {
        ...
        if (user === adminUser) {
            ...
            for (const key in dynamoDBObject) {
                ...
                const pathNameNew = `/${tableName}/read/${value}`;

                try {
                    const res = await API.get(endpoint, pathNameNew, userInit);
    ...
    render() {
        const table = NAIL_PRODUCTS_COLUMN_DESCRIPTION;
        const tableProps = NAIL_PRODUCTS_COLUMN_PROPERTIES;
        const tablePropsType = NAIL_PRODUCTS_COLUMN_PROPERTIES_TYPE;
