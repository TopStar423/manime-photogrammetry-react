...
export default class Rotate extends React.Component {
  ...
  render() {
    return (
      <Box position='absolute' width='100%' height='100%' display='flex' justifyContent='center' alignItems='center' bg='rgba(0,0,0,0.7)' onClick={this.onClick}>
        <Box ref={ref => this.modal = ref} width={500} bg='#ffffff' display='flex' flexDirection='row' flexWrap='wrap' zIndex={100}>
          {
            this.props.signedUriArray.map((uri, index) => (
              <Box flex='0 1 50%' pt='50%' border='0.5px solid #000000' position='relative' style={{ boxSizing: 'border-box' }}>
                ...
                <Box width='100%' display='flex' flexDirection='row' justifyContent='space-evenly' mb={2}>
                  <StandardButton onClick={() => this.rotateImage(this.props.latestKeys[index].key, 90)}>Rotate Left</StandardButton>
                  <StandardButton onClick={() => this.rotateImage(this.props.latestKeys[index].key, -90)}>Rotate Right</StandardButton>
                </Box>
              </Box>
            ))
          }
          <Box width='100%' display='flex' flexDirection='row' justifyContent='center' mb={2} mt={2}>
