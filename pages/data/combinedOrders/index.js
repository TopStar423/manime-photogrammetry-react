...
import {
    COMBINED_ORDERS_COLUMN_DESCRIPTION,
    COMBINED_ORDERS_COLUMN_PROPERTIES,
    COMBINED_ORDERS_COLUMN_PROPERTIES_TYPE,
    pathName,
    tableName,
    endpoint
} from '../../../static/constants/combinedOrders';
            ...
            <BoardBody table={table}>
                ...
                <BoardBodyContainer table={table}>
                    <BoardBodyContentDescriptions table={table}>
                        { table.map((item, i) => <div key={i} type='display' style={{ width: '120px', marginLeft: '10px', display: 'inline-block' }} onClick={() => this.sortData(tableProps[i])}>{item}</div>) }
